...
- **The Error Code sensor now ships enabled.** It was disabled by default and had to be turned on per entity, which meant that on a Creator 5 Pro the one channel that says *why* a print stopped was the one nobody had switched on. It shows the raw code from `/detail` — `E0163` — with no mapping to a description; FlashForge's published Creator 5 error table does not list every code firmware actually sends, so a raw code you can search for beats a lookup that silently comes up empty.

  **Existing installations are not affected automatically.** Home Assistant records the entity as disabled when it is first created and does not re-enable it when a later version changes the default, so on an existing setup the sensor stays off until you enable it by hand: **Settings → Devices & Services → FlashForge → your printer → Error Code → enable**. New installations get it switched on.

- **Requires `flashforge-python-api` 1.3.5** (was 1.3.4). It maps the raw status `"pause"` — which is what a Creator 5 Pro sends for a paused print, rather than the documented `"paused"` — so Machine Status and the Paused binary sensor stop reading `unknown` / `off` during exactly the fault above. Also maps `"downloading"` to `BUSY`. Without this floor the fix is half-delivered: the printer flags the problem but nothing says the print is paused.
...
- **Print Completion Time no longer recedes while a print is paused.** The sensor allowed `PAUSED` and `PAUSING`, and the library derives its value as `now() + estimatedTime` on every poll — a conversion that only holds still while the firmware is counting `estimatedTime` down. It freezes that field the moment the print stops advancing, so with one term fixed and the clock still moving the timestamp stepped forward one minute per minute: pause on a clog for an hour and the sensor claimed the print would finish an hour later than it had when the pause began, receding for as long as the pause lasted. Found while checking whether the clog fix above left the ETA sensible. `sensor.<printer>_print_completion_time` is now `unknown` unless the printer is actively printing — heating is excluded for the same reason as the paused states, since the pre-print warmup does not advance the job either and drifts identically, just for minutes rather than hours; `sensor.<printer>_remaining_time` is unaffected and still reports the correct remaining duration throughout a pause, which is the reading that stays meaningful while paused.

  No new library floor is needed: `flashforge-python-api` 1.4.0 (released alongside this) returns `None` here itself, but the state check is kept in the integration so the sensor is correct against 1.3.5 as well. The same derivation was fixed in `ff-5mp-api-py`, `ff-5mp-api-ts`, FlashForgeUI-Electron and FlashForgeWebUI at the same time; it originates in the C# `ff-5mp-api` that every port inherited the line from.

- **The Error binary sensor stayed off while the printer sat on a detected clog.** It asked only whether the machine state was `ERROR`, but a Creator 5 Pro that detects a clog does not enter that state: it pauses the print and fills `errorCode`. Observed live at 89 % of a print (pid 41, firmware 1.9.5) — the printer displayed *"Clog detected"*, `/detail` reported `status: "pause"` with `errorCode: "E0163"`, and Home Assistant showed no problem on any channel: the Error sensor was `off`, the Paused sensor was `off`, and Machine Status read `unknown` — the raw `"pause"` did not map to `PAUSED`, fixed in `flashforge-python-api` 1.3.5 and required below. `binary_sensor.<printer>_error` now also turns on when the printer reports a non-empty error code, whatever state it reports alongside it. Reported and diagnosed by @RedAces.
