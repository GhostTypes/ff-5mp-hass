...
- **Material matching is AD5X-only; the Creator 5 series reports file names and nothing else.** `gcodeListDetail` — the per-tool material data the matching dialog is built from — comes back only on the AD5X. The 5M / 5M Pro *and* the Creator 5 / Creator 5 Pro return bare names, confirmed against real Creator 5 Pro hardware (2026-08-05) after the docs claimed the opposite for three releases. `requires_material_matching()` needs slots *and* `tool_datas`, so a C5 always takes the confirmation path; that is correct behavior, not a bug to route around — and to be precise, that statement is about **material matching**, which we did NOT change. Separately, the *entire* local-job-start flow is now hidden from the card for the Creator 5 series: its firmware cannot start a previously-uploaded local job over HTTP (only a fresh 3mf upload+start works), so the card shows the info message "Local job management is not available on this printer." in place of the file list and Start button. The server-side dispatch (`job.py` → `start_creator5_job`) is unchanged and still works if a client calls it directly — the card is just an untrusted client that no longer offers it. Never infer tools from the slot report or the file name to force the matching dialog open — a mapping the printer never described is a guess sent to a machine. Matching on a C5 is possible only at upload, where the 3mf is parsed locally. Slot swatch entities are unaffected (they come from `/detail`).
