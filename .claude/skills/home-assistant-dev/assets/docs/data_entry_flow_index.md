# Source: https://developers.home-assistant.io/docs/data_entry_flow_index
...
This will show a navigation menu to the user to easily pick the next step. The menu labels can be hardcoded by specifying a dictionary of `{step_id: label}` or translated via `strings.json` under "menu\_options" when specifying a list. Additionally menu descriptions can be provided via `strings.json` under "menu\_option\_descriptions".
...
Passing `sort=True` to async\_show\_menu will also sort the menu items by their label in the user's language.
