# Source: https://developers.home-assistant.io/docs/core/platform/application_credentials
...
| --- | --- | --- | --- |
| authorize\_url | str | **Required** | The OAuth authorize URL that the user is redirected to during the configuration flow. |
| token\_url | str | **Required** | The URL used for obtaining an access token. |
...
| Name | Type |  | Description |
| --- | --- | --- | --- |
| client\_id | str | **Required** | The OAuth Client ID provided by the user. |
| client\_secret | str | **Required** | The OAuth Client Secret provided by the user. |
...
While developing locally, you will need to run `python3 -m script.translations develop` to see changes made to `strings.json` [More info on translating Home Assistant.](/docs/translations)