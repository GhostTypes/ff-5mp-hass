...
Home Assistant has a built-in API which exposes the Assist API to LLMs. This API allows LLMs to interact with Home Assistant via [intents](/docs/intent_builtin), and can be extended by registering intents.
...
| `device_id` | string | The device\_id of the device the user used to initiate the conversation. |
