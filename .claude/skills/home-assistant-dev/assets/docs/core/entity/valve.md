# Source: https://developers.home-assistant.io/docs/core/entity/valve
...
| --- | --- | --- | --- |
| current\_valve\_position | `int | None` | `None` | The current position of the valve where 0 means closed and 100 is fully open. This attribute is required on valves with `reports_position = True`, where it's used to determine state. |
| is\_closed | `bool | None` | `None` | If the valve is closed or not. Used to determine `state` for valves that don't report position. |
| is\_closing | `bool | None` | `None` | If the valve is closing or not. Used to determine `state`. |
| is\_opening | `bool | None` | `None` | If the valve is opening or not. Used to determine `state`. |
| reports\_position | `bool` | **Required** | If the valve knows its position or not. |
