# Source: https://developers.home-assistant.io/docs/core/entity/lock
...
| --- | --- | --- | --- |
| changed\_by | string | None | Describes what the last change was triggered by. |
| code\_format | string | None | Regex for code format or None if no code is required. |
| is\_locked | bool | None | Indication of whether the lock is currently locked. Used to determine `state`. |
| is\_locking | bool | None | Indication of whether the lock is currently locking. Used to determine `state`. |
| is\_unlocking | bool | None | Indication of whether the lock is currently unlocking. Used to determine `state`. |
| is\_jammed | bool | None | Indication of whether the lock is currently jammed. Used to determine `state`. |
| is\_opening | bool | None | Indication of whether the lock is currently opening. Used to determine `state`. |
| is\_open | bool | None | Indication of whether the lock is currently open. Used to determine `state`. |
