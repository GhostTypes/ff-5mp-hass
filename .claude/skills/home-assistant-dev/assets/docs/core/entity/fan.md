# Source: https://developers.home-assistant.io/docs/core/entity/fan
...
| --- | --- | --- | --- |
| current\_direction | `str | None` | `None` | The current direction of the fan. |
| is\_on | `bool | None` | `None` | True if the fan is on. |
...
| preset\_mode | `str | None` | `None` | The current preset\_mode. One of the values in `preset_modes` or `None` if no preset is active. |
| preset\_modes | `list[str] | None` | `None` | The list of supported preset\_modes. This is an arbitrary list of str and should not contain any speeds. |
| speed\_count | `int` | 100 | The number of speeds the fan supports. |
...
Optional. If not implemented will default to checking what method to call using the is\_on property.
