# Source: https://developers.home-assistant.io/docs/asyncio_working_with_async
...
So if you are making calls to the core (the hass object) from within a callback or coroutine, use the methods that start with async\_. If you need to call an async\_ function that is a coroutine, your task must also be a coroutine.

## Implementing an async component

To make a component async, implement an async\_setup.
...
For platforms we support async setup. Instead of setup\_platform you need to have a coroutine async\_setup\_platform.
