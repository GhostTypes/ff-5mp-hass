# Source: https://developers.home-assistant.io/docs/integration_events

> **info**
>
> Rather than emitting events directly on the event bus, integrations are generally encouraged to publish them as [event entities](/docs/core/entity/event) instead. This approach enhances the user experience by making it easier for the user to browse and identify all available events.
...
If a device or service only fires events, you need to [manually register it in the device registry](/docs/device_registry_index#manual-registration).

## Making events accessible to users

A [Device trigger](/docs/device_automation_trigger) can be attached to a specific event based on the payload, and will make the event accessible to users. With a device trigger a user will be able to see all available events for the device and use it in their automations.
