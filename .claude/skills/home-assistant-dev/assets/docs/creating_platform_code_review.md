# Source: https://developers.home-assistant.io/docs/creating_platform_code_review
...
1. Follow our [Style guidelines](/docs/development_guidelines)
...
1. Requirements have been added to [`manifest.json`](/docs/creating_integration_manifest). The `REQUIREMENTS` constant is deprecated.
...
4. Each requirement meets the [library requirements](/docs/api_lib_index#basic-library-requirements).

### 2. Configuration

1. If the platform can be set up directly, add a voluptuous schema for [configuration validation](/docs/development_validation)
...
2. Group your calls to `add\_entities if possible.
3. If the platform adds extra actions, the format should be `<domain of your integration>.<service action name>`. So if your integration's domain is "awesome\_sauce" and you are making a light platform, you would register service actions under the `awesome_sauce` domain. Make sure that your service actions [verify permissions](/docs/auth_permissions#checking-permissions).
...
6. Leverage the [entity lifecycle callbacks](/docs/core/entity#lifecycle-hooks) to attach event listeners or clean up connections.
