# Source: https://developers.home-assistant.io/docs/entity_registry_disabled_by
...
Entity disabling works with entities provided via a config entry or via an entry in configuration.yaml. If your integration is set up via a config entry and supports [unloading](/docs/config_entries_index#unloading-entries), Home Assistant will be able to reload your integration after entities have been enabled/disabled to apply the changes without a restart.
...
## Integrations setting default value of disabled\_by for new entity registry entries
...
## Config entry system options setting default value of disabled\_by for new entity registry entries
...
## Integrations offering options to control disabled\_by
...
Integrations can offer options to users either via [configuration.yaml](/docs/configuration_yaml_index) or using an [Options Flow](/docs/config_entries_options_flow_handler).

If this option is offered by integrations, you should not leverage the disabled\_by property in the entity registry. Instead, if entities are disabled via a config options flow, remove them from the device and entity registry.