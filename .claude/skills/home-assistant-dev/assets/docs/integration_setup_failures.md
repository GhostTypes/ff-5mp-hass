...
If you are using a [DataUpdateCoordinator](/docs/integration_fetching_data#coordinated-single-api-poll-for-data-for-all-entities), calling `await coordinator.async_config_entry_first_refresh()` will also trigger this exception automatically if the first refresh failed.
...
* source: This will always be "SOURCE\_REAUTH"
* entry\_id: The entry\_id of the config entry that needs reauthentication
* unique\_id: The unique\_id of the config entry that needs reauthentication
