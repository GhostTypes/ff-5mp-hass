# Source: https://developers.home-assistant.io/docs/operating-system/boards/odroid-m1s
...
1. Download the *UMS Utility* image: [`ODROID-M1S_EMMC2UMS.img`](https://dn.odroid.com/RK3566/ODROID-M1S/Installer/ODROID-M1S_EMMC2UMS.img). The *UMS Utility* is a special image that switches ODROID-M1S to USB Mass Storage device.
...
From there, copy the binary file from your PC (e.g. `ssh [email protected]:/path_to/ODROID-M1S_EMMC2UMS.img /tmp` - replace the user with your username on the PC, the mypc.local with your computer name or IP address and the path\_to with the actual path to your downloaded binary). This command will then copy the binary to /tmp/ on your HAOS.
...
The Home Assistant OS image is bootable by the SoC directly. Refer to the [boot sequence documentation](https://wiki.odroid.com/odroid-m1s/board_support/boot_sequence) for the details on what part of the boot is executed from the eMMC and what from the SD card. The steps documented above should however cover all scenarios that a standard user may encounter during usage.
...
Odroid-M1S introduces a new 14pin expansion header. Refer to [the ODROID wiki](https://wiki.odroid.com/odroid-m1s/hardware/expansion_connectors).
