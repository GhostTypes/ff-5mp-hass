# Source: https://developers.home-assistant.io/docs/operating-system/configuration
...
* The `network` folder can contain any kind of NetworkManager connection files. For more information see [Network](/docs/operating-system/network).
* The `modules` folder is for modules-load configuration files.
* The `modprobe` folder is for modules configuration files (/etc/modprobe.d)
* The `udev` folder is for udev rules files.
* The `authorized_keys` file activates debug SSH access on port `22222`. See [Debugging Home Assistant](https://developers.home-assistant.io/docs/operating-system/debugging).
* The `timesyncd.conf` file allow you to set different NTP servers. HassOS won't boot without correct working time servers!
* The `hassos-*.raucb` file is a firmware OTA update which will be installed. These can be found on on the [release](https://github.com/home-assistant/hassos/releases/) page.
