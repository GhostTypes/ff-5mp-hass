...
For basic docs, see `parametrize-basics`.
...
:py`Metafunc.parametrize <pytest.Metafunc.parametrize>()`:
...
- you can put `@pytest.mark.parametrize` style
...
`pytest.mark.parametrize ref` decorator to write parametrized tests
