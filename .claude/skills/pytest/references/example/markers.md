...
You can provide one or more `node IDs` as positional
...
attributes set on the test function, markers applied to it or its parents and any [extra keywords](_pytest.nodes.Node.extra_keyword_matches)
...
`adding a custom marker from a plugin`.
...
Skip and xfail marks can also be applied in this way, see `skip/xfail with parametrize`.
