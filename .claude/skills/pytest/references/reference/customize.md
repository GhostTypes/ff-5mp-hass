...
Many `pytest settings` can be set in a *configuration file*, which
...
`tox.ini` files are the configuration files of the [tox](https://tox.readthedocs.io)_ project,
...
`setup.cfg` files are general purpose configuration files, used originally by `distutils` (now deprecated) and :std`setuptools`, and can also be used to hold pytest configuration
...
* Is used by plugins as a stable location to store project/test run specific information;
  for example, the internal `cache` plugin creates a `.pytest_cache` subdirectory
  in `rootdir` to store its cross-test run state.

`rootdir` is **NOT** used to modify `sys.path`/`PYTHONPATH` or
influence how modules are imported. See `pythonpath` for more details.
...
`addopts` inside a configuration file because the `rootdir` is used to *find* the configuration file
already.
...
The [Config](pytest.Config) object (accessible via hooks or through the `pytestconfig` fixture)
