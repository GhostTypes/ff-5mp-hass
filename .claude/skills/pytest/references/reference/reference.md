...
Can be passed to `ids` of :py`Metafunc.parametrize <pytest.Metafunc.parametrize>()`
or to `id` of `pytest.param()` to hide a parameter set from the test name.
...
.. py:function:: pytest.mark.filterwarnings(filter)
    ...
    :keyword str filter:
        ...
        as specified in `python:warning-filter` section of
...
This mark has the same signature as :py`pytest.Metafunc.parametrize()`; see there.
...
.. py:function:: pytest.mark.skipif(condition, *, reason=None)
    ...
    :param condition: `True/False` if the condition should be skipped or a `condition string`.
    :keyword str reason: Reason why the test function is being skipped.
...
.. py:function:: pytest.mark.xfail(condition=False, *, reason=None, raises=None, run=True, strict=strict_xfail)
    ...
    :keyword Union[bool, str] condition:
        ...
        `condition string`). If a `bool`, you also have
        to specify `reason` (see `condition string`).
    :keyword str reason:
...
Will create and attach a [Mark](pytest.Mark) object to the collected
[Item](pytest.Item), which can then be accessed by fixtures or hooks with
`Node.iter_markers <_pytest.nodes.Node.iter_markers>()`. The `mark` object will have the following attributes:
...
`dumps`/`loads` API of the :py`json` stdlib module.
...
**Tutorial**: `monkeypatching`
    ...
    Returns a `~pytest.MonkeyPatch` instance.
...
**members:** 
    :special-members: __getitem__, __iter__, __len
...
**decorator:** 
    ...
    See `writinghooks` and `pluggy.HookimplMarker()`.
...
**decorator:** 
    ...
    See `declaringhooks` and `pluggy.HookspecMarker()`.
...
All runtest related hooks receive a :py[pytest.Item](pytest.Item) object.
...
Objects accessible from `fixtures` or `hooks`
...
Result object used within `hook wrappers`, see :py[Result in the pluggy documentation](pluggy.Result) for more information.

## Stash

    :special-members: __setitem__, __getitem__, __delitem__, __contains__, __len
**members:** 
...
Can be declared at the **global** level in *test modules* to apply one or more `marks` to all
...
.. envvar:: PYTEST_ADDOPTS
   ...
   This contains a command-line (parsed by the py`shlex` module) that will be **prepended** to the command line given
   by the user, see `adding default options` for more information.
...
.. envvar:: PYTEST_CURRENT_TEST
   ...
   processes can inspect it, see `pytest current test env` for more information.
...
.. envvar:: PYTEST_DEBUG_TEMPROOT
   ...
   Root for temporary directories produced by fixtures like `tmp_path`
   as discussed in `temporary directory location and retention`.
...
.. envvar:: PYTEST_DISABLE_PLUGIN_AUTOLOAD
   ...
   explicitly specified in `PYTEST_PLUGINS` or with `-p` will be loaded.
   See also `--disable-plugin-autoload`.
...
.. envvar:: NO_COLOR
   ...
   See [no-color.org](https://no-color.org/)_ for other libraries supporting this community standard.
...
.. confval:: cache_dir
   ...
   `.pytest_cache` which is created in `rootdir`. Directory may be
   relative or absolute path. If setting relative path, then directory is created
   relative to `rootdir`. Additionally, a path may contain environment
   variables, that will be expanded. For more information about cache plugin
   please refer to `cache_provider`.
...
.. confval:: consider_namespace_packages
   ...
   Controls if pytest should attempt to identify [namespace packages](https://packaging.python.org/en/latest/guides/packaging-namespace-packages)_
   ...
   Only [native namespace packages](https://packaging.python.org/en/latest/guides/packaging-namespace-packages/#native-namespace-packages)_
   are supported, with no plans to support [legacy namespace packages](https://packaging.python.org/en/latest/guides/packaging-namespace-packages/#legacy-namespace-packages)_.
...
.. confval:: faulthandler_exit_on_timeout
   ...
   `exit=True` to the `faulthandler.dump_traceback_later()` function. This
...
.. confval:: faulthandler_timeout
   ...
   fixture setup and teardown). Implemented using the `faulthandler.dump_traceback_later()` function,
...
.. confval:: log_cli_date_format
    ...
    Sets a :py`time.strftime()`-compatible string that will be used when formatting dates for live logging.
...
.. confval:: log_cli_format
    ...
    Sets a :py`logging`-compatible string used to format live logging messages.
...
.. confval:: log_date_format
    ...
    Sets a :py`time.strftime()`-compatible string that will be used when formatting dates for logging capture.
...
.. confval:: log_file_date_format
    ...
    Sets a :py`time.strftime()`-compatible string that will be used when formatting dates for the logging file.
...
.. confval:: log_file_format
    ...
    Sets a :py`logging`-compatible string used to format logging messages redirected to the logging file.
...
.. confval:: log_format
    ...
    Sets a :py`logging`-compatible string used to format captured logging messages.
...
.. confval:: markers
    ...
    When the `strict_markers` configuration option is set,
    only known markers - defined in code by core pytest or some plugin - are allowed.

    You can list additional markers in this setting to add them to the whitelist,
    in which case you probably want to set `strict_markers` to `true`
...
.. confval:: pythonpath
   ...
   Paths are relative to the `rootdir` directory.
...
.. confval:: strict_xfail
    ...
    test suite.
    For more information, see `xfail strict tutorial`.
...
.. confval:: testpaths
   ...
   executing pytest from the `rootdir` directory.
...
.. option:: -k EXPRESSION
    ...
    See `select-tests` for more information and examples.
...
.. option:: -m MARKEXPR
    ...
    See `mark` for more information on markers.
...
.. option:: --stepwise, --sw
    ...
    See `cache stepwise` for more information.
...
.. option:: --import-mode
    ...
    * `importlib`: use importlib to import test modules
    ...
    See `pythonpath` for more information.
...
.. option:: --fixtures, --funcargs
    ...
    Fixtures with leading `_` are only shown with `--verbose`.
...
.. option:: -r CHARS, --report-chars=CHARS
    ...
    * `N`: resets the list
    ...
    See `pytest.detailed_failed_tests_usage` for more information.
...
.. option:: --tb=STYLE
    ...
    * `no`: no traceback
    ...
    See `how-to-modifying-python-tb-printing` for examples.
...
.. option:: --doctest-modules
    ...
    See `doctest` for more information on using doctests with pytest.
...
.. option:: --log-cli-level=LEVEL
    ...
    CLI logging level. See `live_logs`.
