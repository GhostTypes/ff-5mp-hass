...
* `NUMBER`: when enabled, floating-point numbers only need to match as far as
  ...
  compared using `pytest.approx()` with relative tolerance equal to the
...
(see `python:doctest.REPORT_UDIFF`, `python:doctest.REPORT_CDIFF`,
`python:doctest.REPORT_NDIFF`, `python:doctest.REPORT_ONLY_FIRST_FAILURE`):
...
Also, the `usefixtures` mark and fixtures marked as `autouse` are supported
