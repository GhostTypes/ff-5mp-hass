...
`pluggy documentation about hook wrappers`.
...
For a real world example, see `newhooks.py`_ from [xdist](https://github.com/pytest-dev/pytest-xdist).
...
because of the standard `validation mechanism`:
...
Plugins often need to store data on `~pytest.Item`\s in one hook
...
So pytest offers a better way to do this, [item.stash](_pytest.nodes.Node.stash).
...
Stashes are available on all node types (like `~pytest.Class`,
`~pytest.Session`) and also on `~pytest.Config`, if needed.
