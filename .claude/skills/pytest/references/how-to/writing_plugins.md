...
including third party projects.  Please refer to `using plugins` if you
...
* `external plugins`: installed third-party modules discovered
  through `entry points` in their packaging metadata
...
4. by loading all plugins registered through installed third-party package
   `entry points`, unless the
   `PYTEST_DISABLE_PLUGIN_AUTOLOAD` environment variable is set.
...
6. by loading all "initial" `conftest.py` files:
   ...
   - determine the test paths: specified on the command line, otherwise in
     `testpaths` if defined and running from the rootdir, otherwise the
     current dir
   - for each test path, load `conftest.py` and `test*/conftest.py` relative
     ...
     in its `pytest_plugins` variable if present.
...
All of these plugins implement `hooks` and/or `fixtures`
...
Also consider `contributing your plugin to pytest-dev<submitplugin>`
...
done via a `302` import hook which gets installed early on when
...
This will allow you to use the :py[pytester](pytest.Pytester)
