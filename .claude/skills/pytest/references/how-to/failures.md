...
The `faulthandler` standard module
can be used to dump Python tracebacks on a segfault or after a timeout.

The module is automatically enabled for pytest runs, unless the `-p no:faulthandler` is given
on the command-line.

Also the `faulthandler_timeout=X<faulthandler_timeout>` configuration option can be used
...
in a `__del__ <object.__del__>()` implementation.

Unhandled thread exceptions are exceptions raised in a `~threading.Thread`
...
The warnings may be silenced selectively using the `pytest.mark.filterwarnings ref`
mark. The warning categories are `pytest.PytestUnraisableExceptionWarning` and
