...
operators. (See `tbreportdemo`).  This allows you to use the
...
`excinfo` is an `~pytest.ExceptionInfo` instance, which is a wrapper around
...
* The `match` parameter also matches against [PEP-678](https://peps.python.org/pep-0678/)_ `__notes__`.
...
They both supply a method `pytest.RaisesGroup.matches()` `pytest.RaisesExc.matches()` if you want to do matching outside of using it as a :external+python:std`context manager`. This can be helpful when checking `.__context__` or `.__cause__`.
...
You can also use the `excinfo.group_contains() <pytest.ExceptionInfo.group_contains>()`
...
* Using `pytest.mark.xfail` with the `raises` parameter is probably better for something
