...
:py`@pytest.fixture <pytest.fixture>()` invocation
...
will discover and call the :py`@pytest.fixture <pytest.fixture>()`
...
their teardown code, as `the email examples above` showed.
...
Let's pull `an example from above`, and tweak it a
...
Fixture functions can accept the :py[request](_pytest.fixtures.FixtureRequest) object
...
Using the :py[request](_pytest.fixtures.FixtureRequest) object, a fixture can also access
...
through the special :py[request](pytest.FixtureRequest) object:
...
:py`@pytest.fixture <pytest.fixture>()`, a list of values
...
Usually projects that provide pytest support will use `entry points`,
