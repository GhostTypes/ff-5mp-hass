...
* `prepend` (default): The directory path containing each module will be inserted into the *beginning*
  of :py`sys.path` if not already there, and then imported with
  the `importlib.import_module <importlib.import_module>()` function.
...
* `append`: the directory containing each module is appended to the end of :py`sys.path` if not already
  ...
  we advocate for using `src-layouts`.

  Same as `prepend`, requires test module names to be unique when the test directory tree is
  not arranged in packages, because the modules will be put in :py`sys.modules` after importing.
...
* `importlib`: this mode uses more fine control mechanisms provided by `importlib` to import test modules, without changing :py`sys.path`.
  ...
  * Testing utility modules in the tests directories (for example a `tests.helpers` module containing test-related functions/classes)
  ...
  1. Given a certain module path, for example `tests/core/test_models.py`, derives a canonical name
     ...
     For non-test modules, this will work if they are accessible via :py`sys.path`. So
  ...
  2. If the previous step fails, we import the module directly using `importlib` facilities, which lets us import it without
     ...
     Because Python requires the module to also be available in :py`sys.modules`, pytest derives a unique name for it based
     on its relative location from the `rootdir`, and adds the module to :py`sys.modules`.
...
change :py`sys.path` in order to import test modules or `conftest.py` files, and the issues users
...
:py`sys.path` (if not there already) in order to load
...
:py`sys.path` in order to import `test_foo.py` as the *module* `test_foo`. The same is done
with the `conftest.py` file by adding `root/foo` to :py`sys.path` to import it as `conftest`.
...
equivalent behaviour, except that the latter will add the current directory to :py`sys.path`, which
