...
In testing, a [fixture](https://en.wikipedia.org/wiki/Test_fixture#Software)
...
test (see `test-anatomy`). In pytest, they are functions you define that
...
:py`@pytest.fixture <pytest.fixture>()`. Here's a simple example of
...
In addition, pytest continues to support `xunitsetup`.  You can mix
...
pytest does not do any special processing for [SIGTERM](signal.SIGTERM) and
`SIGQUIT` signals ([SIGINT](signal.SIGINT) is handled naturally
...
see [this comment](5243#issuecomment-491522595) in the issue
